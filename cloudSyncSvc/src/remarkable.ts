...
export default class Remarkable {
  ...
  public async refreshToken() {
    ...
    const { body }: { body: string } = await got.post(
  ...
  public async getStorageUrl({
    ...
    const body: { Host: string; Status: string } = await this.client
      .get(
  ...
  public async register({
    ...
    if (!code) {
      throw new Error(
        "Must provide a code from https://my.remarkable.com/connect/desktop",
      );
    }
    ...
    const generateDeviceId = () => {
      const fingerprint =
        os.platform() + os.arch() + os.hostname() + os.cpus()[0].model;
      const namespace = new Array(10)
        .fill(0)
        .concat(...toUint8Array(getMac().replace(/:/g, "")));
  ...
  public async createDirectory(name: string, ID: string, parent?: string) {
    ...
    if (!body[0].Success || !body[0].BlobURLPut) {
      console.log("Create directory response: ", body[0]);
      throw new Error("Error during the creation of the upload request");
    }
    ...
    const folderMetadata = Object.assign({}, defaultFolderMetadata);
